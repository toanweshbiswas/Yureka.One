        ...
        <div ref={statsRef} className="grid grid-cols-1 gap-8">
          {METRICS.map((metric, i) => (
            <div key={metric.label} className="text-center">
              <div className="text-[clamp(32px,10vw,48px)] font-light leading-none tracking-[-0.04em] text-white">
                <ScrambleIn text={metric.value} delay={i * 200} triggered={statsInView} />
              </div>
              <div className="mt-3 text-[13px] font-bold tracking-wide text-[#5fae52]">
                {metric.label}
              </div>
            </div>
          ))}
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          {FEATURES.map((feature) => (
            <div key={feature.title}>
              ...
              <div className="text-[12px] leading-relaxed text-white/40">{feature.desc}</div>
            </div>
          ))}
        </div>
      </section>
