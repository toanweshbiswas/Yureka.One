...
For members and waitlist applicants on **app.yureka.one**.
...
1. Go to [app.yureka.one/login](https://app.yureka.one/login).
2. Use email + password **or** Continue with Google.
3. After login:
   - **Accepted** → Dashboard
   - **Pending / on-hold / rejected** → Waiting room (`/waiting`)
   - **No waitlist row** → treat as not yet applied; join waitlist

**Forgot password:** use reset on login; email comes from Yureka support address.

---

## 3. Waiting room

- Shows queue state while the team reviews your application.
- When accepted, refresh or re-login → you are routed to `/dashboard`.
- Rejected / on-hold: contact support with the email you applied with (see Support SOPs for reply scripts).
...
| **Bills** | Bill-like recurring spend |
| **Referrals** | Your code and referred friends |
| **Profile** | Name / phone / preferences |

### Connect Gmail for spend

1. Open Expenses (or Bills) → allow **Gmail read-only** when prompted.
2. First sync can take a minute.
3. **Force resync** is limited (**5 times / 15 days**). Soft refresh does not burn that quota.

### Add to Home Screen (PWA)

1. On phone Safari/Chrome, use “Add to Home Screen”.
2. Open the icon for the app-like experience (browse / Super Browse works best here).

---

## 5. Self-serve troubleshooting

| Symptom | What to try |
|---------|-------------|
| Stuck on waiting after “I was approved” | Hard refresh; sign out/in; confirm you use the **same email** as waitlist |
